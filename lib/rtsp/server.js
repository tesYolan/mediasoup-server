...
class RtspServer extends EventEmitter {
	constructor(webRtcServer){
        ...
        .on('new-connection', (connection) => {
        	connection
        	.on('new-stream', (stream, roomId) => {
        		let source = this.getSource(stream.peer.id, connection, roomId);
	...
	addSource(source, roomId) {
		this.sources[source.id] = source;
		this.emit('new-source', source, roomId);
	}

	getSource(sourceId, connection = null, roomId) {
		if(connection !== null && !this.sources[sourceId]){
			this.addSource(new RtspSource(sourceId, connection), roomId);
		}
